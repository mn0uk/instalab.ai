...
export default function NewHypothesisPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">From hypothesis to experiment plan</h1>
        <p className="text-slate-600 mt-1">
          Submit a scientific hypothesis. The system runs a literature novelty check, then generates
          a protocol, materials list, timeline, and validation plan.
        </p>
      </div>

      <form onSubmit={onSubmit} className="space-y-4 bg-white border border-slate-200 rounded-lg p-6">
        <div>
          ...
          <textarea
            ...
            placeholder="State an intervention, a measurable outcome with a threshold, and a control."
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
            placeholder="e.g. diagnostics, gut health, cell biology, climate"
          />
        </div>
        ...
        >
          {createMutation.isPending ? "Submitting..." : "Generate experiment plan"}
        </button>
      </form>

      <div className="bg-white border border-slate-200 rounded-lg p-6">
        <h2 className="text-sm font-semibold text-slate-700 mb-3">Try a sample hypothesis</h2>
